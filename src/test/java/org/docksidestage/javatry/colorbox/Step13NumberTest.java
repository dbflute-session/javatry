...
public class Step13NumberTest extends PlainTestCase {
    ...
    public void test_countZeroToFiftyFour_IntegerOnly() {
        ...
        long target = colorBoxList.stream()
                .flatMap(box -> box.getSpaceList().stream())
                .map(space -> space.getContent())
                .filter(content -> content instanceof Integer && (int) content >= 0 && (int) content <= 54 )
                .count();
    ...
    public void test_countZeroToFiftyFour_Number() {
        ...
        long target = colorBoxList.stream()
                ...
                .map(space -> space.getContent())
                .filter(content -> {
                    return content instanceof Integer && (int) content >= 0 && (int) content <= 54
                            || content instanceof Float && (float) content >= 0 && (float) content <= 54;
                })
                .count();
    ...
    public void test_findColorBigWidthHasInteger() {
        List<ColorBox> colorBoxList = new YourPrivateRoom().getColorBoxList();
        Integer target = colorBoxList.stream()
                .flatMap(box -> box.getSpaceList().stream())
                .map(space -> space.getContent())
    ...
    public void test_sumBigDecimalInList() {
        List<ColorBox> colorBoxList = new YourPrivateRoom().getColorBoxList();
        Stream<Object> target = colorBoxList.stream()
                .flatMap(box -> {
                    log("-----------");
                    return box.getSpaceList().stream();})
                .map(space -> space.getContent())
                .filter(content -> {
                    log("=====HI");
